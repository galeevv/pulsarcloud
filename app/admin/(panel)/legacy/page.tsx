          ...
          <AdminCard title="Пользователи" description="Последние 50">
            ...
            <div className="mt-3 flex justify-between gap-2">
              <Link
                ...
                href={`/admin/legacy?tab=users&page=${Math.max(1, page - 1)}&q=${encodeURIComponent(query)}&subscriptionStatus=${encodeURIComponent(subscriptionStatus)}`}
              >
                ← Назад
              </Link>
              ...
              <Link
                className="text-sm text-muted-foreground"
                href={`/admin/legacy?tab=users&page=${page + 1}&q=${encodeURIComponent(query)}&subscriptionStatus=${encodeURIComponent(subscriptionStatus)}`}
              >
                Далее →
              </Link>
            </div>
